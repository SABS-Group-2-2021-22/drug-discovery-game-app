...
class Descriptors extends React.Component {
  ...
  render() {
    ...
    const HA_value = this.props.mols[this.props.mol_id].data.descriptors.HA;
    ...
    //set all styles to red (these will be changed based on thresholds set in next block of code)
    let HA_color = "red";
    let MW_color = "red";
    ...
    //change color based on the thresholds for if value is good, bad or neutral
    if (HA_value >= 6) {
      HA_color = "green";
    } else if (HA_value >= 4) {
      HA_color = "yellow";
